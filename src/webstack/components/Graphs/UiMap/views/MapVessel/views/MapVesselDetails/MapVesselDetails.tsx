...
const MapVesselDetails: React.FC<IVesselDetailsProps> = ({ vessel, setVessel, onResize }) => {
  ...
  const initialDimensions = isDesktop ? { width: 400, height: height } : { width: width - 30, height: 500 };
  ...
  const closeVessel = useCallback(() => {
    if (vesselDetailsRef?.current?.style) vesselDetailsRef.current.style = undefined;
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (startY.current !== null) {
      ...
      const isMobileDetailsArea = (Number(translateY.current) * -1) + (vesselDetailsRef.current.offsetHeight / 2) < height /2;
      translateY.current = currentY.current - startY.current;
      if (vesselDetailsRef.current && isMobileDetailsArea) {
        vesselDetailsRef.current.style.transform = `translateY(${translateY.current}px)`;
      }
    }
  };
  ...
  const handleTouchEnd = () => {
    ...
    if (startY.current !== null && currentY.current !== null) {
      ...
      if (deltaY > 50) {
        ...
        if (vesselDetailsRef.current) {
          vesselDetailsRef.current.style.transform = 'translateY(0)';
        }
      }
    }
    startY.current = null;
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (vesselDetailsRef.current) {
      onResize?.(newSize);
      if (isDesktop) {
        vesselDetailsRef.current.style.width = `${newSize}px`;
      } else {
        vesselDetailsRef.current.style.height = `${newSize}px`;
      }
      if (newSize < 100) closeVessel();
    }
  }, [dimensions, isDesktop, onResize, closeVessel]);

  const handleMouseMove = useCallback((e: MouseEvent) => {
    if (isDragging && vesselDetailsRef.current) {
      if (isDesktop) {
        const newWidth = (width - 30) - e.clientX;
        setDimensions((prev: any) => ({ ...prev, width: newWidth }));
        vesselDetailsRef.current.style.width = `${newWidth}px`;
        onResize?.(newWidth);
        if (newWidth < 100) closeVessel();
      } else {
        const newHeight = height - e.clientY;
        setDimensions((prev: any) => ({ ...prev, height: newHeight }));
        vesselDetailsRef.current.style.height = `${newHeight}px`;
        onResize?.(newHeight);
        if (newHeight < 100) closeVessel();
      }
    }
  ...
  return (
    <>
      ...
      <div
        ...
        style={isDesktop ? { width: dimensions.width, height: height } : { width: width == 0 ? 400 : width, height: dimensions.height }}
      >
