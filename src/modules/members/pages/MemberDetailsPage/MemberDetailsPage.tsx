...
import AdaptTableCell, {
  NaCell,
} from "@webstack/components/AdapTable/components/AdaptTableContent/components/AdaptTableCell/AdaptTableCell";
...
import DetailsModal from "../../views/MemberDetailsModal/MemberDetailsModal";
...
export default function MemberDetailsPage({ memberId }: IProps) {
  ...
  const showOrderHistoryModal = DetailsModal();
    ...
    const localSearch = (newLicenses: License[] | null) =>
      newLicenses?.filter((item: any) => {
        for (const key of Object.keys(item)) {
          if (item?.keywords.toLowerCase().includes(searchCriteria.toLowerCase()) && !returnData.includes(item))
  ...
  const getMemberProfileInformation = useCallback(async () => {
                    ...
                    return (
                      <div key={index + key} className="report-details_subheader-column">
                        ...
                        <div className="report-details_subheader-value">
                          {!["phone", "email"].includes(key) && (value ? value.toUpperCase() : <NaCell />)}
                          {key === "phone" &&
  ...
  }, [header?.title, memberId, memberService, setHeader]);

  const getMemberLicenses = useCallback(async () => {
    ...
    return formatted;
  }, [licenseService, memberId]);

  const getMemberOrderHistory = useCallback(async () => {
  ...
  }, [memberId, shoppingService]);
  useEffect(() => {
    if (licenseSearch === "") getMemberLicenses();
    if (!orderHistory) getMemberOrderHistory();
    getMemberProfileInformation();
  }, [getMemberOrderHistory, getMemberLicenses, getMemberProfileInformation, licenseSearch, orderHistory]);
  ...
  return (
    <>
      ...
      <br />
      <AdapTable
        options={{ index: 0, hide: "footer", tableTitle: "Licenses", title: memberId }}
        filters={filters}
        filterBy={["type"]}
        setFilter={(e) => {
          e === RESET_FILTER_KEY ? getMemberLicenses() : setLicenseSearch(e);
        }}
