#!/usr/bin/env sh
# MindBurn — Apple-Silicon Mac entry installer (one-liner target; NO secrets inside).
#   curl -fsSL https://deepturn.com/download/mac.sh | sh -s -- --token <jwt>
...
[ "$(uname -s)" = "Darwin" ] && [ "$(uname -m)" = "arm64" ] \
  || { echo "This installer is for Apple-Silicon Macs; detected $(uname -s)/$(uname -m)." >&2; exit 1; }
